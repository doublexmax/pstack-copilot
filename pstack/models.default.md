...
Copilot splits what Cursor encoded in one slug into two fields on the `task` tool: `model`
and `reasoning_effort`. Every row below gives both.
...
## Cursor slug translation

Kept for reading upstream pstack diffs.

| Cursor slug | `model` | `reasoning_effort` |
| --- | --- | --- |
| grok-4.5-fast-xhigh | `grok-4.5` | `high` |
| gpt-5.6-sol-max | `gpt-5.6-sol` | `xhigh` |
| claude-opus-5-thinking-xhigh | `claude-opus-5` | `xhigh` |
| claude-fable-5-thinking-max | `gemini-3.1-pro-preview` | `high` |
| inherit-parent, auto | omit both fields | omit |

Upstream's fable slot has no Copilot equivalent. It is replaced by Gemini rather than by a
second Anthropic model, to keep four distinct vendors in the panel roles.
