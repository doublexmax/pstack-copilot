...
The [Opening a PR playbook](../../skills/poteto-mode/playbooks/opening-a-pr.md) runs [`unslop`](../../skills/unslop/SKILL.md) over the diff before each commit and over the PR description and commit bodies. Ask for the same outcome in plain words when a diff needs more than prose cleanup: remove narrating comments, unsupported guards, dead compatibility paths, and unrelated edits.
