...
upstream benny was built on cursor's event-triggered automations and a slack intake. this fork keeps the behavior and swaps the plumbing: workflows are **scheduled and polled** via `save_workflow`, and the intake is an **adapter** that defaults to azure devops work items. see [`references/intake.md`](./references/intake.md).
...
| upstream | here |
| --- | --- |
| cursor automation, event-triggered | copilot workflow, scheduled and polled |
| `/automate` (creation only) | `save_workflow` (creates **and** updates via `workflow_id`) |
| `.cursor/automations/benny/` | `.github/automations/benny/` |
