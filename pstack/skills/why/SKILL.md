...
Before spawning investigators, list the MCP servers actually available in this session. Read the tool list you were given: MCP tools are namespaced by server, so the distinct prefixes are the enabled servers. Do not assume a server exists because the category below names one.
...
- `model`: your configured why-investigators model (default `grok-4.5 / high`)
- **Do not use `agent_type: "explore"` here.** It has no MCP access, which disables MCP-backed investigators entirely. The source control investigator would survive it, but keep the roster uniform. Investigators still shouldn't write anything. That's a posture the prompt enforces, not a sandbox.
...
- `agent_type`: `"general-purpose"`
- `model`: your configured why-synthesizer model (default `gemini-3.1-pro-preview / high`)
- Not `explore`. The synthesizer's quality check spot-verifies citations, which can require MCP access.
