...
1. Locate the prior trail. A prior session in the local session store (query it with the `session_store_sql` tool at `source: "local"`, scoping to the active repository so you never read another project's private conversations), a cloud-session URL, or a pushed branch. Read `sessions.summary` and the last few `turns` first, then scan back for the decision points. Do the scan in a subagent and keep the reduced timeline in the main thread (the **principle-guard-the-context-window** skill).
