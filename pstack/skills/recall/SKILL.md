...
Your own history lives in the local session store, queried with the `session_store_sql` tool at `source: "local"`. Tables: `sessions` (id, cwd, repository, branch, summary, created_at), `turns` (session_id, turn_index, user_message, assistant_response, timestamp), `session_files` (files touched), and `session_refs` (commits, PRs, issues). Scope by `repository` or `cwd` rather than reading everything. SQLite syntax, and time filters must compare 10-char date prefixes on both sides (`substr(created_at, 1, 10) >= date('now', '-7 days')`).
...
2. Lock the scope before searching. Pin the window ("recent" is a real range, default the last 7 days), the topic if named, and the repository (default the active one; never read another project's sessions without being asked, which here is a `WHERE repository = ...` clause). State the scope back. Never quietly turn "all" into "recent N".
3. Fan out across your chat history. Start with one `session_store_sql` query that lists candidate sessions in scope, ordered by `created_at` descending, selecting only `id`, `summary`, and `branch`. Then spawn parallel subagents on a fast, cheap model, each taking a slice of that candidate list, since reading turns is grunt work. Tell every subagent to filter by `session_id` first and select only the columns it needs, since the store is columnar and partitioned on `session_id`. Skip the current session plus obvious noise (subagent, eval, and test sessions). Each returns the same schema, one block per session: topic, the user's goal, decisions, open threads, struggles and corrections, and artifacts (PRs, tickets, branches), each citing the session id. For one or two sessions, skip the fan-out and query directly. The raw turns stay in the subagents. The main thread gets only their findings.
...
5. Verify against live state. A stored turn or a stale ticket is history, not current truth, so take the PRs, branches, and tickets that the mining and the sweep surfaced and check them with `git`, `gh`, or the Azure DevOps MCP. When the answer hinges on what an agent actually did (the tools it ran, files it read, errors it hit), query `session_files` and `session_refs` for that `session_id` rather than inferring from the prose.
