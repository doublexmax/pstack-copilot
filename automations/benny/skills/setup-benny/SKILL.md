...
`save_workflow` can update in place, unlike the creation-only automation command upstream used. Use `list_workflows` to find the existing `benny-triage` and `benny-reproduce`, then call `save_workflow` with the returned `workflow_id` and only the fields that change. Never create a second workflow that does the same job.
...
8. A second `run_workflow` on the same already-handled report is a no-op. This is the check that upstream did not need, because upstream fired once per event and Copilot polls on a schedule.
