...
- `inbox/` holds completion pointers. `gates.md` parks human gates (question, options, default on no answer) so a completion flood cannot wipe a pending `ask_user` state.
...
VERIFY       exact commands or the project's `.github/skills/verify-*` path, plus known gotchas
...
- Drain in batches at four points: the end of a critical section, a track rollup, a frontier watcher wake (arm it under autopilot mode with a long heartbeat poll), and before a human report. Begin each batch with `orch inbox drain`. Arrivals during a drain wait for the next one.
...
- The frontier is a computed object, never narrative. Recompute `frontier.json` after every merge and topology change by walking `targetRefName` from `master` over `repo_pull_request action=list`: ordered PR list, branch names, head SHAs, a generation number, the lowest unmerged PR. Target refs are the only record of the chain — there is no stacking tool holding a second copy — so a PR whose parent branch no longer exists is a break to report, not a gap to infer around.
- Exactly one stacker per chain may write target refs, serialized within its chain; record the holder in the standing orders. Rebases run in cloud; a local rebase at this scale takes the laptop down.
- Workers never rebase and never write a target ref. Babysitters follow `playbooks/babysit.md`, one per chain, scoped to one immutable frontier generation; they report conflicts to the stacker rather than rebasing.
- PR abandons and retargets go through the stacker only. ADO retargets nothing for you: completing a base PR leaves every PR above it pointing at that branch, and completing it with `deleteSourceBranch` on orphans them outright. Merges and topology surgery are units with briefs like any other.
...
- Never resume an agent to check on it; a resume restarts an idle agent. Probe read-only: the ledger, `units.tsv`, `repo_pull_request action=list`, pushed branches, `get_session` for a spawned session. Store mtime is not liveness.
