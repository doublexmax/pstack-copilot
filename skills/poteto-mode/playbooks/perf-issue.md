...
1. Capture a baseline trace on the matching surface via the project's `.github/skills/verify-*` skill when one exists, otherwise the repo's own profiler/harness or a generated verify skill.
...
3. Plan the fix from the trace. If it crosses a function boundary, `architect` first. Delegate implementation to a subagent using your configured perf-issue model (default `gpt-5.6-sol / xhigh`); review the diff. Capture a post-fix trace.
