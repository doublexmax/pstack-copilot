...
6. **Verify the chain from stored turns, not self-report.** Query each candidate's conversation with the `session_store_sql` tool at `source: "local"`, scoped to that candidate's `session_id`, and read `session_files` for what it actually opened. Citing a principle is not reading its leaf skill, and reading it is not applying it. Grade chain-following from the files it really read plus the shape of the code, never from the candidate's own claims.
