...
At the end of the run, before handing back, check the log told the truth. Query this run's turns with the `session_store_sql` tool at `source: "local"`, scoped to your own `session_id`. Walk the log against what actually happened:
